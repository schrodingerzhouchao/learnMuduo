...
struct Localtime
{
    ...
    Localtime(time_t offset, bool dst, int arrb)
        : gmOffset(offset), isDst(dst), arrbIdx(arrb)
    {
    }
};
...
public:
  ...
  int32_t readInt32()
  {
      ...
      // be32toh e.g., convert values between host and big-/little-endian byte order
      // htobenn host byte order -> big-endian order
      // htolenn
      // benntoh
      // lenntoh
      // nonstandard
...
bool readTimeZoneFile(const char *zonefile, struct TimeZone::Data *data)
{
    ...
    if (f.valid())
    {
        try
        {
            // 44-octet header followed by a variable-length body transition times and leap
            // second occurrences.
            string head = f.readBytes(4);
            // Time Zone Information Format(TZif)
            // https://tools.ietf.org/id/draft-murchison-tzdist-tzif-00.html
            if (head != "TZif")
                throw logic_error("bad head");
            string version = f.readBytes(1);
            f.readBytes(15);

            int32_t isutccnt = f.readInt32();   // UTC/local
            int32_t isstdcnt = f.readInt32();   // standard/wall
            int32_t leapcnt = f.readInt32();    // leap seconds
            int32_t timecnt = f.readInt32();    // transition times
            int32_t typecnt = f.readInt32();    // local time type
            int32_t charcnt = f.readInt32();    // total number used by the set of time zone                                           // designaions contained in the body
...
time_t TimeZone::fromLocalTime(const struct tm &localTm) const
{
    ...
    if (localTm.tm_isdst)
    {
        struct tm tryTm = toLocalTime(seconds - local->gmOffset);
    ...
    return seconds - local->gmOffset;
}

struct tm TimeZone::toUtcTime(time_t secondsinceEpoch, bool yday)
{
    struct tm utc;
    memZero(&utc, sizeof(utc));
    utc.tm_zone = "GMT";
    int seconds = static_cast<int>(secondsinceEpoch % kSecondsPerDay);
    int days = static_cast<int>(secondsinceEpoch / kSecondsPerDay);
    if (seconds < 0)
    {
...
{
    return fromUtcTime(utc.tm_yday + 1900, utc.tm_mon + 1, utc.tm_mday,
